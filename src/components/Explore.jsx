...
const Explore = ({ title, placesAPI }) => {
    return (
        <>
            <div className='realtive my-7 md:mt-3'>
                <div className='roamify-container'>
                        ...
                        {placesAPI?.map((val, i) => (
                            <div key={i} className='flex items-center gap-5 sm:gap-3 rounded-lg hover:bg-emerald-300 transition-all duration-300 cursor-pointer hover:scale-105'>
                                <div className='flex items-center justify-center'>
                                    <img src={val.placeImg} alt={val.location} className='w-20 h-20 sm:h-16 sm:w-16 rounded-lg filter drop-shadow-lg ' />
                                </div>
